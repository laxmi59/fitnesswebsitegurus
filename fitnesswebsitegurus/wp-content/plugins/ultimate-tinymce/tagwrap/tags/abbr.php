<head>
<title>{#tagwrap_dlg.titleAbbr}</title>
...
<div class="yinstr">
    <p>{#tagwrap_dlg.noteAbbr}</p>
    <p>{#tagwrap_dlg.noteAbbr2}</p>
</div>
...
{#tagwrap_dlg.noteAbbr1} <input id="title_value" type="text" name="title" width="200px" value="" />
<br /><br />
</div>
...
<div class="mceActionPanel">
  ...
  <input type="button" id="insert" name="insert" value="{#insert}" onClick="jwl_pass_form_data();tinyMCE.execCommand('mceInsertContent',false,'<abbr title=\'' + jwl_title + '\'>' + jwl_sel_content4 + '</abbr>');tinyMCEPopup.close();" />&nbsp;<input type="button" id="cancel" name="cancel" value="{#cancel}" onClick="tinyMCEPopup.close();" />
  </div>
</div>
</form>
